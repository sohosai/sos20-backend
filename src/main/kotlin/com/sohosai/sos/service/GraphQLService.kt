        ...
        if (request.operationName !in allowedOperationWithoutToken) {
            if (authStatus == null) {
                ...
                return when (authStatus.cause) {
                    is AuthenticationFailedCause.NoCredentials -> UnauthorizedExecutionResult("No token")
                    is AuthenticationFailedCause.InvalidCredentials -> UnauthorizedExecutionResult("Invalid token")
                    is AuthenticationFailedCause.Error -> UnauthorizedExecutionResult(authStatus.cause.cause)
                }
            }
        }
